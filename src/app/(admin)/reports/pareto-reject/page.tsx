...
import { getParetoNGReport } from "@/server/reportService";
...
import { ParetoNGChart } from "@/components/reports/pareto-ng-chart";
import { paretoNGTableColumns } from "./pareto-ng-table-columns";
import { DataTable } from "@/components/ui/data-table";
import { useDocumentTitle } from "@/hooks/useDocumentTitle";
import { exportToExcel, formatDataForExport } from "@/lib/excel";

export default function ParetoNGReportPage() {
  useDocumentTitle("Pareto NG Report");
  ...
  // Fetch Pareto NG data with SWR
  const cacheKey = useMemo(
    () =>
      `pareto-ng-${format(startDate, "yyyy-MM-dd")}-${format(endDate, "yyyy-MM-dd")}-${selectedPlantId}-${selectedLineId}-${selectedShiftId}`,
    [startDate, endDate, selectedPlantId, selectedLineId, selectedShiftId],
  );

  const { data: reportData, isLoading } = useSWR(cacheKey, async () => {
    const result = await getParetoNGReport({
      startDate,
      endDate,
  ...
  return (
    <div className="flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Identify top rejection categories and quality issues
            </p>
          </div>
      ...
      <div className="flex-1 space-y-4 sm:space-y-6 p-4 sm:p-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>NG Details</CardTitle>
            <CardDescription>
              Detailed breakdown of rejection events by category
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <DataTable
                columns={paretoNGTableColumns}
                data={reportData.tableData}
                showColumnToggle={true}
                onExport={handleExport}
                exportFilename="pareto-ng-report"
              />
            )}
          </CardContent>
        </Card>
