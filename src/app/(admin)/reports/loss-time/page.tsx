...
import { getLossTimeReport } from "@/server/reportService";
...
import { LossTimeChart } from "@/components/reports/loss-time-chart";
import { lossTimeTableColumns } from "./loss-time-table-columns";
import { DataTable } from "@/components/ui/data-table";
import { useDocumentTitle } from "@/hooks/useDocumentTitle";
import { exportToExcel, formatDataForExport } from "@/lib/excel";

export default function LossTimeReportPage() {
  useDocumentTitle("Loss Time Report");
  ...
  // Fetch loss time data with SWR
  const cacheKey = useMemo(
    () =>
      `loss-time-${format(startDate, "yyyy-MM-dd")}-${format(endDate, "yyyy-MM-dd")}-${selectedPlantId}-${selectedLineId}-${selectedShiftId}`,
    [startDate, endDate, selectedPlantId, selectedLineId, selectedShiftId],
  );

  const { data: reportData, isLoading } = useSWR(cacheKey, async () => {
    const result = await getLossTimeReport({
      startDate,
      endDate,
  ...
  return (
    <div className="flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Analyze unplanned downtime and production efficiency
            </p>
          </div>
      ...
      <div className="flex-1 space-y-4 sm:space-y-6 p-4 sm:p-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Loss Time Details</CardTitle>
            <CardDescription>
              Detailed breakdown of production time and downtime by work order
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <DataTable
                columns={lossTimeTableColumns}
                data={reportData.tableData}
                showColumnToggle={true}
                onExport={handleExport}
                exportFilename="loss-time-report"
              />
            )}
          </CardContent>
        </Card>
