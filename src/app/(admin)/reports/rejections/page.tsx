...
import { getRejectionReport } from "@/server/reportService";
...
import { RejectionChart } from "@/components/reports/rejection-chart";
import { rejectionTableColumns } from "./rejection-table-columns";
import { DataTable } from "@/components/ui/data-table";
import { useDocumentTitle } from "@/hooks/useDocumentTitle";
import { exportToExcel, formatDataForExport } from "@/lib/excel";

export default function RejectionReportPage() {
  useDocumentTitle("Rejection Report");
  ...
  // Fetch rejection data with SWR
  const cacheKey = useMemo(
    () =>
      `rejection-${format(startDate, "yyyy-MM-dd")}-${format(endDate, "yyyy-MM-dd")}-${selectedPlantId}-${selectedLineId}-${selectedShiftId}`,
    [startDate, endDate, selectedPlantId, selectedLineId, selectedShiftId],
  );

  const { data: reportData, isLoading } = useSWR(cacheKey, async () => {
    const result = await getRejectionReport({
      startDate,
      endDate,
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Analyze product rejections and quality issues
            </p>
          </div>
      ...
      <div className="flex-1 space-y-4 sm:space-y-6 p-4 sm:p-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Detailed breakdown of rejection events by work order
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <DataTable
                ...
                exportFilename="rejection-report"
              />
            )}
          </CardContent>
        </Card>
